...
public class Laporan_Pembayaran extends AppCompatActivity {
    ...
    TabelAdapterPembayaran adapter;
    Button btnpdf;
    int no = 1;
    Button cari;
    EditText min, max;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        rv.setLayoutManager(new LinearLayoutManager(Laporan_Pembayaran.this));
        ...
        cari.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Query query = db.child("Pembayaran").orderByChild("tglbyr");
    ...
    private void showListenerRange(DataSnapshot dataSnapshot, Date date_minimal, Date date_maximal) {
        ...
        for (final DataSnapshot ds : dataSnapshot.getChildren()) {
            Model_Pembayaran pem = ds.getValue(Model_Pembayaran.class);
            if (pem != null) {
        ...
        adapter = new TabelAdapterPembayaran(listPembayaran, Laporan_Pembayaran.this);
        rv.setAdapter(adapter);
    }


    private void RefreshList() {
        db.child("Pembayaran").orderByChild("penghuni").addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
    ...
    private void showListener(DataSnapshot dataSnapshot) {
        ...
        for (final DataSnapshot ds : dataSnapshot.getChildren()) {
            ...
            no = no++;
            ide = pem.getId();
            penghuni = pem.getPenghuni();
            unit = pem.getUnit();
            tglbyr = pem.getTglbyr();
            harga = pem.getHarga();
        }
        adapter = new TabelAdapterPembayaran(listPembayaran, Laporan_Pembayaran.this);
        rv.setAdapter(adapter);
